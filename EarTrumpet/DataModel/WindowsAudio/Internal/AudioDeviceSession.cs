...
{
    internal class AudioDeviceSession : BindableBase, IAudioSessionEvents, IAudioDeviceSession,
        IAudioDeviceSessionInternal
    {
        ...
        public AudioDeviceSession(IAudioDevice parent, IAudioSessionControl session, Dispatcher foregroundDispatcher)
        {
            ...
            _appInfo = AppInformationFactory.CreateForProcess(ProcessId, true);
        ...
        public IAudioDevice Parent
        {
            get
            {
                if (_parent.TryGetTarget(out var parent)) return parent;
                return null;
            }
        }
        ...
        public SessionState State
        {
            get
            {
                ...
                if (_isMoved) return SessionState.Moved;
        ...
        void IAudioSessionEvents.OnSimpleVolumeChanged(float NewVolume, int NewMute, ref Guid EventContext)
        {
            ...
            StorageFactory.GetSettings("PersistVolume").Set(Parent.DisplayName + "." + ExeName, NewVolume);
        ...
        void IAudioSessionEvents.OnGroupingParamChanged(ref Guid NewGroupingParam, ref Guid EventContext)
        {
            ...
            _dispatcher.BeginInvoke((Action) (() => { RaisePropertyChanged(nameof(GroupingParam)); }));
        }
        ...
        void IAudioSessionEvents.OnStateChanged(AudioSessionState NewState)
        {
            ...
            _dispatcher.BeginInvoke((Action) (() => { RaisePropertyChanged(nameof(State)); }));
        }
        ...
        void IAudioSessionEvents.OnDisplayNameChanged(string NewDisplayName, ref Guid EventContext)
        {
            ...
            _dispatcher.BeginInvoke((Action) (() => { RaisePropertyChanged(nameof(DisplayName)); }));
        }
        ...
        void IAudioSessionEvents.OnChannelVolumeChanged(uint ChannelCount, IntPtr afNewChannelVolume,
            uint ChangedChannel, ref Guid EventContext)
        {
            var channelVolumesValues = new float[ChannelCount];
            Marshal.Copy(afNewChannelVolume, channelVolumesValues, 0, (int) ChannelCount);

            for (var i = 0; i < ChannelCount; i++) _channels.Channels[i].SetLevel(channelVolumesValues[i]);
        }
        ...
        ~AudioDeviceSession()
        {
            try
            {
                if (_isRegistered) _session.UnregisterAudioSessionNotification(this);
            }
            catch (Exception ex)
            {
